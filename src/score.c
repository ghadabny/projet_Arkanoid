#include "score.h"
#include "globals.h" // Include if needed for global variables like screen dimensions
...
void drawScoreboard(SDL_Surface* surface, int currentScore, int highScore) {
    ...
    drawText(surface, "SCORE:", 10, SCOREBOARD_HEIGHT / 2 - FONT_HEIGHT / 2);
    drawScore(surface, currentScore, 200, SCOREBOARD_HEIGHT / 2 - FONT_HEIGHT / 2);
    drawText(surface, "HIGH SCORE:", surface->w - 300, SCOREBOARD_HEIGHT / 2 - FONT_HEIGHT / 2); // Adjust X as needed
    drawScore(surface, highScore, surface->w - 40, SCOREBOARD_HEIGHT / 2 - FONT_HEIGHT / 2);
}
